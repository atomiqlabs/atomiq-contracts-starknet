...
const assertionTable = [
    ...
    [32, (buffer, index_be, noValue) => [ "buffer.read_u256_be("+index_be+")", noValue ? null : "0x"+buffer.subarray(index_be, index_be+32).toString("hex") ], "u256_be"],
...
function getRandomAccessPanicTest(type) {
    ...
    return "#[test]\n" +
        ...
        "    let mut serialized_byte_array = "+toCairoSerializedByteArray(buffer)+".span();\n"+
...
function getRandomByteArrayTest() {
    ...
    const buffer = crypto.randomBytes(length);
    const def = "let mut serialized_byte_array = "+toCairoSerializedByteArray(buffer)+".span();\n"+
        "let buffer = Serde::<ByteArray>::deserialize(ref serialized_byte_array).unwrap();\n";
    ...
    assertions.push("assert_eq!(buffer.hash_sha256(), "+buffer256bitToU32Arr(crypto.createHash("sha256").update(buffer).digest())+");");
    assertions.push("assert_eq!(buffer.hash_dbl_sha256(), "+buffer256bitToU32Arr(crypto.createHash("sha256").update(crypto.createHash("sha256").update(buffer).digest()).digest())+");");
    for(let i=0;i<5;i++) {
